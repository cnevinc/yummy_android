...
public class SyncManager {
	...
	public void getPresetArticlesFromLocalJson(String fileName , final SyncObserver adapter,final View view){
        ...
        try {
			...
			// mActivity.getActionBar().setSubtitle(formatter.format(Setting.TOTAL_ROWS)+ "個結果");
			for (int i = 0; i < entries.length(); i++) {
	...
	public void searchArticlesByKeyword(final SyncObserver adapter, String keyword, final View view){
		...
		String api_url = "http://emma.pixnet.cc/blog/articles/search?category=27&key="
				+ URLEncoder.encode(keyword+" 食譜")+"&per_page="+Setting.PER_PAGE + "&page="+Setting.CURRENT_PAGE;
